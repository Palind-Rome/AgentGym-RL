...
# Leave empty to reuse ref_log_prob as teacher_log_probs for a cheap sanity run.
# Set to a teacher checkpoint path to launch a separate teacher policy worker.
teacher_model_path=""
...
if [ -n "${teacher_model_path}" ]; then
    teacher_override="algorithm.distillation.teacher_model.path=${teacher_model_path}"
fi
...
HYDRA_FULL_ERROR=1 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True WANDB_MODE=offline python3 -m verl.agent_trainer.main_ppo  \
    ...
    algorithm.distillation.loss_coef=${opd_loss_coef} \
